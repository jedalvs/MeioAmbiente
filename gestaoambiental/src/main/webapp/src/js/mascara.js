...
function rg(v) {
	v = v.replace(/\D/g, ""); // Remove tudo o que não é dígito
	v = v.replace(/(\d)(\d{7})$/, "$1.$2"); // Coloca o . antes dos últimos 3
											// dígitos, e antes do verificador
	v = v.replace(/(\d)(\d{4})$/, "$1.$2"); // Coloca o . antes dos últimos 3
											// dígitos, e antes do verificador
...
function valorFormatado(v) {
	...
	v = v.replace(/(\d)(\d{2})$/, "$1,$2");// coloca a virgula antes dos 2
											// últimos dígitos
	return v;
}
